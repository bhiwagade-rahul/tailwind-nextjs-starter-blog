...
items = ["entertainment/box-office", "entertainment/hollywood", "cinema"]
urls = []

for every_item in items:
    START_URL = "https://www.pinkvilla.com/" + every_item
    driver.get(START_URL)
    driver.implicitly_wait(40)
    time.sleep(5)
    articles = driver.find_elements(By.XPATH, "//div//a[contains(@href," + every_item + ") and @title]")
    article_count = 0
    max_articles = 10
    ...
    for article in articles:
        ...
        if article.get_attribute("href") is not None and len(article.get_attribute("href").replace(START_URL, "")) > 100:
            print("====>> " + str(article.get_attribute("href")))
            urls.append(article.get_attribute("href"))
            print(f"📰 Found {article.get_attribute("href")} link")
            article_count += 1
